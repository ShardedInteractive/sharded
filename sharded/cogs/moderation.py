...
class Moderation(commands.Cog):
    ...
    @commands.hybrid_command(name='purge', description="Purge a specified amount of messages from the channel.")
    @app_commands.guilds(GUILD_ID)
    @commands.has_permissions(manage_messages=True)
    async def purge(self, ctx, amount: int):
        deleted = await ctx.channel.purge(limit=amount)
        await ctx.send(f"Purged {len(deleted)} messages from this channel.", ephemeral=True)
    ...
    async def kick(self, ctx, member: discord.Member, *, reason: str):
        if ctx.author.top_role <= member.top_role:
            await ctx.send("You cannot kick a member with a higher or equal role than you.", ephemeral=True)
            return
    ...
    async def unban(self, ctx, user_id: str, *, reason: str = None):
        try:
            ...
            await ctx.send(f"Successfully unbanned {user.mention}" + (f" for: {reason}" if reason else ""))
